...
def test_mask_covers_the_moving_blob_and_nothing_else():
    ...
    mask = motion_pixel_mask(recording.frames, reference)
    width = mask.shape[1]
...
def test_threshold_is_the_mean_plus_one_standard_deviation():
    ...
    assert np.array_equal(motion_pixel_mask(recording.frames, 1), peak_change >= threshold)


def test_a_pixel_that_never_moves_is_dropped():
    frames = frames_with_bright_pixels([[], [(0, 0)], [(0, 0)], [(0, 0)]])
    mask = motion_pixel_mask(frames, reference_frame=1)
    assert mask[0, 0]
    assert mask.sum() == 1
...
def test_end_frame_limits_which_frames_are_accumulated():
    ...
    assert not motion_pixel_mask(frames, 1, mask_end_frame=2, legacy=True)[3, 3]
    assert motion_pixel_mask(frames, 1, mask_end_frame=2, legacy=False)[3, 3]


def test_legacy_stops_one_frame_early():
    frames = frames_with_bright_pixels([[], [(0, 0)], [(3, 3)], []])
    assert motion_pixel_mask(frames, 1, mask_end_frame=2, legacy=True).sum() == 1
    assert motion_pixel_mask(frames, 1, mask_end_frame=2, legacy=False).sum() == 2


def test_start_frame_skips_earlier_frames():
    frames = frames_with_bright_pixels([[], [(0, 0)], [(3, 3)], []])
    assert not motion_pixel_mask(frames, 1, mask_start_frame=2)[0, 0]
    assert motion_pixel_mask(frames, 1, mask_start_frame=2)[3, 3]
...
def test_positions_count_without_the_reference_frame():
    ...
    frames = frames_with_bright_pixels([[(0, 0)], [], [(3, 3)], []])
    mask = motion_pixel_mask(frames, reference_frame=2, mask_start_frame=2, mask_end_frame=3)
    assert mask[3, 3]
    assert not mask[0, 0]
...
def test_reading_from_disk_gives_the_same_mask(tmp_path):
    ...
    assert np.array_equal(motion_pixel_mask(on_disk, 1), motion_pixel_mask(recording.frames, 1))


def test_mask_is_reported(caplog):
    recording = synthetic_recording(noise=0.01, seed=0)
    with caplog.at_level("INFO", logger="boamotion.traces"):
        motion_pixel_mask(recording.frames, 1)
...
def test_reference_frame_outside_the_recording():
    ...
    with pytest.raises(ValueError, match="outside the recording"):
        motion_pixel_mask(frames, reference_frame=9)


def test_start_frame_must_be_one_based():
    frames = frames_with_bright_pixels([[], [(0, 0)]])
    with pytest.raises(ValueError, match="1-based"):
        motion_pixel_mask(frames, 1, mask_start_frame=0)
